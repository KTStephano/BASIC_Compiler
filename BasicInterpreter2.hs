...
trigCheck = "(define test '((100 let x = asin(1))" ++
                           "(200 let y = acos(1))" ++
                           "(300 let z = atan(1))" ++
                           "(400 print x)" ++
                           "(500 print y)" ++
...
lastCheck = "(define test '((100 let x = right$ (\"heywhatsuphello\" 5))" ++
                           "(200 print x)))"
...
getSubstrLast = do
  (VString str, VIntegral amount) <- binary
  let newStr = lastN (fromInteger amount) str
  return $ VString $ reverse $ newStr

lastN n xs = if n > (length xs) then xs else take n (reverse xs)
