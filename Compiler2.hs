...
data Basic = Statement {getLine' :: Int, getName :: String, body :: Basic} | Expression Sexpr deriving (Eq)

instance Show Basic where
    show (Statement l n b) = show l ++ ": " ++ "{" ++ n ++ " -> " ++ show b ++ "}"
    show (Expression e) = show e


variable :: Int -> Sexpr -> [Basic]
variable line s = translate (Cons (Number line) (Cons (Symbol "let") s))

translate :: Sexpr -> [Basic]
translate (Cons (Number i) c@(Cons (Symbol s) s')) = if s `elem` statementDeclarations then [Statement i s $ Expression s']
                                                   else variable i c
translate (Cons s s') = translate s ++ translate s'
translate Nil = []
...
statementDeclarations = ["dim", "end", "for", "goto", "gosub", "if", "input", "let", "next", "print", "print!", "rem", "return"]
